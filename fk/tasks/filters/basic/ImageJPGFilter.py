...
class ImageJPGFilter(Task[float]):
    ...
    def process(self, context: ImageContext) -> bool:
        ...
        jpeg_quality = self.get_jpg_quality(image)
        if jpeg_quality >= 0:
            return jpeg_quality >= self.quality

        return True

    def get_jpg_quality(self, image: PIL.Image.Image) -> int:
        qsum = 0
        ...
        if len(qdict) >= 1:
            ...
            hash, sums = ImageJPGFilter._HASH_1, ImageJPGFilter._SUMS_1

            if len(qdict) >= 2:
                qvalue += qdict[1][0] + qdict[1][-1]
                hash, sums = ImageJPGFilter._HASH_2, ImageJPGFilter._SUMS_2
